  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {tags.map((tag) => (
        <div
          key={tag.id}
          className="relative bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow p-5"
          onMouseEnter={() => setHoveredTag(tag.id)}
          onMouseLeave={() => setHoveredTag(null)}
        >
          <div className="flex items-start justify-between">
            ...
            <div className="flex space-x-1">
              <button
                type="button"
                onClick={() => onEdit(tag)}
                className="p-1.5 rounded-full text-gray-400 hover:text-indigo-600 hover:bg-gray-100 transition-colors"
              >
