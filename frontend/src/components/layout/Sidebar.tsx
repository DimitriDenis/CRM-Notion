...
export function Sidebar({ isOpen, setIsOpen, isMobile = false, id = 'sidebar' }: SidebarProps) {
  ...
  const sidebarClasses = `
    ...
    fixed inset-y-0 left-0 z-30 flex flex-col bg-white border-r border-gray-200 shadow-md 
    ...
    <div id={id} className={sidebarClasses}>
      ...
      <div className="flex h-16 items-center justify-center border-b border-gray-200 px-6">
        <Link href="/" className="flex items-center space-x-3">
          ...
          {(isOpen || isMobile) && (
            <span className="text-xl font-semibold text-gray-900">NotionCRM</span>
          )}
        </Link>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4">
        <nav className="space-y-1">
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                <IconComponent 
                  className={`h-5 w-5 shrink-0 ${active ? 'text-blue-600' : 'text-gray-500 group-hover:text-gray-700'}`} 
                  aria-hidden="true" 
                />
                {(isOpen || isMobile) && (
        ...
        <div className="mt-10">
          <p className={`mb-2 px-3 text-xs font-semibold text-gray-400 uppercase ${!isOpen && !isMobile ? 'text-center' : ''}`}>
          ...
          <nav className="space-y-1">
            {secondaryNavigation.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon 
                    className={`h-5 w-5 shrink-0 ${active ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-700'}`} 
                    aria-hidden="true" 
                  />
                  {(isOpen || isMobile) && (
                    <span className="ml-3">{item.name}</span>
